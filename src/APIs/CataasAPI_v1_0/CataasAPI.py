...
class CataasAPI:
    ...
    ### Usage Example:
        # Create a CataasAPI object
        `api = CataasAPI()`

        # Generate a cat image URL with optional parameters
        `cat_url = api.getCat(tag="cute", gif=True, text="Hello, world!", width=500, height=300)`

        # Send a GET request to retrieve the cat image
        `response = api.send_request(cat_url)`

        # Access the generated cat image URL
        `generated_cat_url = api.generated_cat`
    """
    ...
    def getCat(self, url:str, tag:str, gif:bool, text:str, type_cat:str, filter_cat:str, width:int, height:int) -> str:
        """
        Construct a URL for retrieving a cat image with optional parameters.
